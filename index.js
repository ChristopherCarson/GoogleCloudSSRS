...
exports.getReport = async (req, res) => {
    ...
    try {
        ...
        const parameterString = parameters ? `&${Object.entries(parameters)
        .map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`)
        .join('&')}` : ""

        const reportUrl = `${ssrsUrl}/ReportServer?${encodeURIComponent(reportPath)}&rs:Command=Render&rs:Format=CSV${parameterString}`;
